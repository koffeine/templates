...
import static koffeine.configuration.security.JsonAuthenticationFilterConfigurer.jsonAuth;
...
//			.logout(AbstractHttpConfigurer::disable) // FOR OAuth 2.0
			...
			// JSON LOGIN
			.with(jsonAuth(), customizer -> customizer
				.loginProcessingUrl("/api/login")
				.successHandler((request, response, authentication) ->
					response.setStatus(HttpServletResponse.SC_OK))
				.failureHandler((request, response, exception) ->
			...
			.logout(customizer -> customizer
				.logoutRequestMatcher(PathPatternRequestMatcher.withDefaults().matcher(HttpMethod.POST, "/api/logout")) // FORM LOGOUT
				.logoutRequestMatcher(new AndRequestMatcher( // JSON LOGOUT
